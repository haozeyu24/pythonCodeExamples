...
# first create the name of the two variables and store them im pair1_trim and pair2_trim. The useful output of trim_galore should be ending with "_val_1.fq.gz" and "_val_2.fq.gz"
# However, if these files are present (return True when using os.path.isfile(pair1_trim)), I would still try to create a new one since I do not know if these outputs were good or bad.
...
def trim_reads(args):
	...
	# The output of trim_galore are redirected to devnull
...
def align_reads(args):
	...
	#Here args.read1 and args.read2 are defined in main(), if the user does not skip trim, the results of trim_reads() will be passed to these two variables.
	...
	#Note that there are two magic "-" in the next command, the first dash is input, and second is output
	...
	with open(align_log, 'a') as log, open(filtered_bam, 'w') as bam_out:
		...
		# did not understand the following code
...
def remove_duplicate_reads(args):
	...
	logging.info('Labeling the duplicated reads using picard tools')
	...
	#samtools view -b in.bam chr1 chr2 ..... chrX ,chrY 
	filter_cmd.extend(['chr{}'.format(c) for c in list(map(str, range(1,23))) + ['X','Y']])
	index_for_rmdup_cmd = ['samtools', 'index', rmdup_bam] ##added by Vincent 
	...
	if os.path.isfile(markdup_bam):
		with open(rmdup_bam, 'w') as bam_out:
			...
			subprocess.call(index_for_rmdup_cmd) ##There should be a step to add index file for rmdup_bam
	else:
		raise FileNotFoundError('{} not found!'.format(markdup_bam))
	return
    
def generate_TagAlign_file(args):
	md_bam = args.output_prefix + '.filt.md.bam'
	rmdup_bam = args.output_prefix + '.filt.rmdup.bam'
	tagalign_file = args.output_prefix + '.filt.rmdup.tagAlign.gz'
	logging.info('Writing Bed file from BAM: format Chr Start End Name_index')
...
# mito and duplicated read percentage
	...
	logging.info('Calling peaks using MACS2. Peak file could be provided later for the second iteration')
	...
	logging.info('Getting reads that are in the peaks')
...
# reads in promoter and promoter usage
	...
	logging.info('Getting reads that are in the promoters')
	...
	logging.info('Getting the name of the promoters, if the promoter files contain the gene name assocaited with the promoters. Therefore it is better to provide a comprehensive promoter GTF file')
	...
	for line in tss_cmd2.stdout:
		line = line.decode()
		fields = line.rstrip().split('\t')
		tss_used[fields[3]].append(fields[7])
#getting read in the blacklist
	logging.info('Getting reads that are in the blacklist regions')
	blacklist_counts= {bc:0 for bc in qc_metrics.index}
	count_blacklist_cmd = subprocess.Popen(['bedtools', 'intersect', '-a', tagalign_file, '-b', args.blacklist_file ], stdout=subprocess.PIPE)
	uniq_blacklist = subprocess.Popen(['uniq'], stdin=count_blacklist_cmd.stdout, stdout=subprocess.PIPE)
	for line in uniq_blacklist.stdout:
		line = line.decode()
		fields = line.rstrip().split('\t')
		blacklist_counts[fields[3]] += 1
	
	logging.info('Starting to write qc metrics.')
	qc_metrics['reads_in_promoters'] = qc_metrics.index.map(tss_counts).fillna(0).astype(int)
	qc_metrics['reads_in_blacklist'] = qc_metrics.index.map(blacklist_counts).fillna(0).astype(int)
	...
	qc_metrics['frac_reads_in_blacklist'] = qc_metrics['reads_in_blacklist'].div(qc_metrics['unique_usable_reads']).replace(np.inf, 0).fillna(0)
...
#I have problem in this step. "Duplicated reads" always give me Keyerror
	#qc_metrics['frac_duplicated_reads'] = qc_metrics['duplicated_reads'].div(qc_metrics['total_sequenced_reads']).replace(np.inf, 0).fillna(0)
	qc_metrics.to_csv(os.path.join(args.output_prefix + '.qc_metrics.txt'), sep='\t')
	logging.info('QC_matrix_done.')
	return	
...
def generate_matrix(args):
	...
	windows_file = filter_distal_peaks(args)
	window_intersect = intersect_regions(tagalign_file, windows_file)
	
	
	logging.info('Generating lf_mtx_file.')
	...
	logging.info('Generating matrix barcodes and regions.')
	...
	with open(tmp_R, 'w') as tR:
		...
		print('''sparse.data <- with(data, sparseMatrix(i=as.numeric(as.factor(V2)), j=as.numeric(as.factor(V1)), x=V3, dimnames=list(levels(as.factor(V2)), levels(as.factor(V1)))))''', file=tR)
...
#depending on the GTF file, for example, ENSEMBL chromesomes are just numbers, however, UCSC chromosomes have "chr"
	#awk_cmd = ['awk', '''BEGIN{{FS=OFS="\t"}} {{peakid=$1":"$2"-"$3; gsub("chr","",peakid); print $1, $2, $3, peakid}}''', regions]
	awk_cmd = ['awk', '''BEGIN{{FS=OFS="\t"}} {{peakid=$1":"$2"-"$3; print $1, $2, $3, peakid}}''', regions]
...
def filter_distal_peaks(args):
	logging.info('Generating windows file in bed format.')
	blacklist_cmd = ['bedtools', 'intersect', '-a',args.distal_peaks, '-b', args.blacklist_file, '-v']
	windows_file = 'filtered_distal_peaks.bed'
	with open(windows_file, 'w') as f:
		subprocess.call(blacklist_cmd, stdout=f)
	return windows_file
...
def main(args):
	...
	if not args.skip_rmdup:
		logging.info('Removing duplicate and mitochrondrial reads.')
		remove_duplicate_reads(args)
	if not args.skip_tagalign:
		logging.info('Generating fragments TagAlign File.')
		generate_TagAlign_file(args)
	if not args.skip_qc:
		logging.info('Generating QC_matrics.')
		qc_metrics(args)
	if not args.skip_matrix:
		logging.info('Generating tagalign, fragment file and chromatin accessibility matrix.')
...
def process_args():
	...
	align_group.add_argument('-t', '--threads', required=False, type=int, default=6, help='Number of threads to use for alignment [8]')
	...
	align_group.add_argument('-ref', '--reference', required=False, type=str, default='/Users/sun_lab_mac_pro/Desktop/Genome_assembly_tools/Genomes/Homo_sapiens_NCBI_GRCh38/Homo_sapiens/NCBI/GRCh38/Sequence/BWAIndex/genome.fa', help='Path to the BWA indexed reference genome')
	
	dup_group = parser.add_argument_group('Remove duplicates arguments')
	dup_group.add_argument('--picard', required=False, type=str, default='/Users/sun_lab_mac_pro/Desktop/Genome_assembly_tools/picard.jar', help='Path to picard.jar')
	...
	matrix_group.add_argument('--distal-peaks', required=False, type=str, default='/Volumes/sequencing/Single_cell_ATACseq/Vincent_Analysis/resources/1-3_year_control_peaks/all_combined/all_combined_peaks_sorted_merged500_distal.bed', help='the distal peaks used to cluster cells')
	matrix_group.add_argument('--chrom-sizes', required=False, type=str, default='/Users/sun_lab_mac_pro/Desktop/Genome_assembly_tools/Genome_sizes/hg38.chrom.sizes.txt', help='Chromosome sizes file from UCSC')
	matrix_group.add_argument('--blacklist-file', required=False, type=str, default='/Users/sun_lab_mac_pro/Desktop/Genome_assembly_tools/Blacklists/hg38-blacklist.v2.bed.gz', help='BED file of blacklisted regions')
	matrix_group.add_argument('--promoter-file', required=False, type=str, default='/Users/sun_lab_mac_pro/Desktop/Genome_assembly_tools/promoter_list/GRCh38_promoter_with_ENSEMBLE_GeneID_1K.bed.gz', help='BED file of autosomal promoter regions')
	...
	skip_group.add_argument('--skip-tagalign', required=False, action='store_true', default=False, help='Skip Tagalign fragment generation step')
